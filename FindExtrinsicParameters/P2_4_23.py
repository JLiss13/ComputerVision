...
E_23=[[-0.01825828, -0.70171135 , 0.05748237],
 [ 0.69614149, -0.0312915 , -0.111097  ],
 [-0.04664882 , -0.06083347 ,  0.01233273]]
...
U_23,S_23,V_T_23=np.linalg.svd(E_23)
S_23[2]=0
S_23=np.diag(S_23)
E_23=np.matmul(np.matmul(U_23,S_23),V_T_23)
U_23,S_23,V_T_23=np.linalg.svd(E_23)
r_L_23=V_T_23[2,:]
print("r_L_23 \n")
print(r_L_23)
...
RL_1=np.matmul(np.matmul(U_23,W),V_T_23)
RL_2=np.matmul(np.matmul(U_23,W.T),V_T_23)
RL_1=RL_2
XL_l_list=np.genfromtxt(os.path.join(PhotoDir,"Match23_pts1.txt"),delimiter=",")
XL_l_list=np.hstack((XL_l_list,np.ones((np.shape(XL_l_list)[0],1))))
XL_l_list=np.matrix(XL_l_list)
XR_r_list=np.genfromtxt(os.path.join(PhotoDir,"Match23_pts2.txt"),delimiter=",")
...
for i in range(np.shape(XL_l_list)[0]):
    ...
    XR_r=XR_r_list[i, :]
    Pz_L=f_l*(np.dot((f_r*RL_1[0,:]-XR_r[0]*RL_1[2,:]),r_L_23))/(np.dot((f_r*RL_1[0,:]-XR_r[0]*RL_1[2,:]),XL_l_c))
    Pz_L_mat[i]=int(Pz_L)
print("R_R_L\n")
print(RL_1)
print("Pz_L_23\n")
...
r_skew=[[0, -r_L_23[2], r_L_23[1]],[r_L_23[2],0,-r_L_23[0]],[-r_L_23[1],r_L_23[0],0]]
...
for i in range(np.shape(XR_r_list)[0]):
    ...
    XL_l_list_temp=np.array(np.matmul(RL_1,XL_l_list_C)+0.76577905*np.reshape(np.dot(RL_1,r_L_23),(3,1)))# Perform rotation + translation
    # XL_l_list_temp=np.array(np.matmul(RL_1,XL_l_list_C)+np.reshape(np.dot(RL_1,r_L_23),(3,1)))# Perform rotation + translation
...
print("XR_r_list_new \n")
print(XR_r_list_new)
plt.title("Reprojection of Match Pts from Image 2 onto Image 3")
img2=cv2.cvtColor(img2,cv2.COLOR_RGB2BGR)
cv2.imwrite(os.path.join(PhotoDir,"P2_4_Reproject_23.JPG"),img2)
print(np.abs(np.average(error)))