...
# #Obtain Keypoints
# Kp1 = np.genfromtxt(os.path.join(PhotoDir, 'Scene_KeyMat_1a.txt'), delimiter=',', skip_header=0, dtype=(None))
# Kp2 = np.genfromtxt(os.path.join(PhotoDir, 'Scene_KeyMat_1b.txt'), delimiter=',', skip_header=0, dtype=(None))
# Kp3 = np.genfromtxt(os.path.join(PhotoDir, 'Scene_KeyMat_1c.txt'), delimiter=',', skip_header=0, dtype=(None))

# #Obtain Descriptors
# des1 = np.genfromtxt(os.path.join(PhotoDir, 'Scene_DesMat_1a.txt'), delimiter=',', skip_header=0, dtype=(None))
# des2 = np.genfromtxt(os.path.join(PhotoDir, 'Scene_DesMat_1b.txt'), delimiter=',', skip_header=0, dtype=(None))
# des3 = np.genfromtxt(os.path.join(PhotoDir, 'Scene_DesMat_1c.txt'), delimiter=',', skip_header=0, dtype=(None))
...
Keymat2=PL.KeyPoints(PhotoDir,'Scene_Undis&Cal_1b',Kp2)
...
np.savetxt(os.path.join(PhotoDir, 'mp_array2_points.txt'), mp_array2, delimiter=',')
...
plt.title('Points from Scene 1a and Scene 1b')
...
# F,mask_f=cv2.findFundamentalMat(mp_array1,mp_array2,cv2.FM_RANSAC, 3.5, .99) #4th param is pixel disparity threshold & 5th is confidence percent
F,mask_f=cv2.findFundamentalMat(mp_array1,mp_array2,cv2.FM_RANSAC, 3, .99) #4th param is pixel disparity threshold & 5th is confidence percent
...
pts_1_omit=[1,11,21,22,28,29,38,41,42,51,53] #Points to omit because of part 4 analysis
pts2 = mp_array2[mask_f.ravel()==1]
pts1 = np.delete(pts1,(pts_1_omit),axis=0) #omit bad apples
pts2 = np.delete(pts2,(pts_1_omit),axis=0) #omit bad apples
...
pts1=np.int32(pts1)
pts2=np.int32(pts2)
np.savetxt(os.path.join(PhotoDir,'Match12_pts1.txt'),pts1,delimiter=',')
np.savetxt(os.path.join(PhotoDir,'Match12_pts2.txt'),pts2,delimiter=',')
...
plt.imshow(img5)
plt.title("Image 1a with Inliers, Outliers and Epipolar Lines")
img5=cv2.cvtColor(img5,cv2.COLOR_RGB2BGR)
cv2.imwrite(os.path.join(PhotoDir,"Scene_1a1b_Epi.JPG"),img5)
#Image 2 with Epi
x,y=np.shape(pts2)
plt.figure()
...
cv2.imwrite(os.path.join(PhotoDir,"Scene_1b1a_Epi.JPG"),img7)