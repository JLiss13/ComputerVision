...
for i in range(x) :
    cv2.circle(MatchedImg, (int(mp_array1[i,0]),int(mp_array1[i,1])), 10, (255,0,0),thickness=50, lineType=8, shift=0)
...
for i in range(x) :
    cv2.circle(MatchedImg, (5184+int(mp_array2[i, 0]), int(mp_array2[i, 1])), 10, (255, 0, 0), thickness=50, lineType=8, shift=0)
...
plt.show()
plt.title('Points from Scene 1a and Scene 1c')
...
F,mask_f=cv2.findFundamentalMat(mp_array1,mp_array2,cv2.FM_RANSAC, 1, .5) #4th param is pixel disparity threshold & 5th is confidence percent
print("F \n")
print(F)
...
pts1=np.int32(pts1)
pts2=np.int32(pts2)
np.savetxt(os.path.join(PhotoDir,'Match13_pts1.txt'),pts1,delimiter=',')
np.savetxt(os.path.join(PhotoDir,'Match13_pts2.txt'),pts2,delimiter=',')
...
cv2.imwrite(os.path.join(PhotoDir,"Scene_1a1c_Epi.JPG"),img5)
#Image 2 with Epi
x,y=np.shape(pts2)
plt.figure()
...
cv2.imwrite(os.path.join(PhotoDir,"Scene_1c1a_Epi.JPG"),img7)